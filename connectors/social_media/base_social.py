...
class SocialMediaConnector(BaseConnector):
    ...
    def _handle_api_error(self, response) -> None:
        ...
        if response.status_code == 401:
            raise SocialMediaAuthenticationError(f"Authentication failed for {self.platform_name}")
        elif response.status_code == 429:
            raise SocialMediaAPIError(f"Rate limit exceeded for {self.platform_name}")
            ...
            raise SocialMediaAPIError(f"{self.platform_name} API error: {error_message}")
