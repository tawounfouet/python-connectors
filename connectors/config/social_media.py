...
class LinkedInConfig(ConnectorConfig):
    ...
    client_secret: Optional[str] = Field(None, description="Secret client de l'application LinkedIn")
    default_visibility: SocialMediaVisibility = Field(
        SocialMediaVisibility.PUBLIC, 
        description="Visibilité par défaut des posts"
    )
...
class YouTubeConfig(ConnectorConfig):
    ...
    access_token: Optional[str] = Field(None, description="Token d'accès OAuth pour les opérations d'écriture")
...
class SocialMediaConfig(BaseModel):
    ...
    def get_configured_platforms(self) -> list[str]:
        ...
        """
        platforms = []
        for platform in ['twitter', 'linkedin', 'facebook', 'instagram', 'youtube', 'tiktok']:
...
def create_social_config_from_dict(config_dict: Dict[str, Any]) -> SocialMediaConfig:
    ...
    global_config = {k: v for k, v in config_dict.items() 
                    if k not in ['twitter', 'linkedin', 'facebook', 'instagram', 'youtube', 'tiktok']}
